...
All 8 behavioral tests pass at every optimization level (`-O0` through `-Os`), for 8 × 5 = 40 cases. The last two are regression tests rather than coverage of an operation: each was written after a defect that the rest of this suite had run past, and each was confirmed to fail against a build with its own fix reverted --- `alias.c` at `-O2` and `-O3`, `livejump.c` at `-O1` and above --- while every other test still passed. That is the gap they exist to close; how the two defects were found is reported in @sec-embench-perf.
...
#figure(
  embench-size-table,
  caption: [Embench-IoT static code size (`.text` bytes, `-O2`) and rvsc1 expansion ratios, #size-stamp.],
) <tbl-embench-size>

Across the suite, synthesis inflates code size by a geometric mean of #sym.times #geo-static() relative to the `gcc17` baseline. The custom `rvsc2` target produces code identical to `gcc17` for all 19 benchmarks (geomean ratio = #geo-sc2-static()), which is why it is not given a column of its own in @tbl-embench-size. The per-benchmark ratio tracks how much of each workload's own code the target cannot express directly: `ud`, which performs most of its arithmetic inside library routines rather than in inline shifts, expands only #sym.times #ratio-static("ud"), whereas `nettle-aes`, whose round function is a chain of constant-amount rotations and byte extractions that each unroll into straight-line code, expands #sym.times #ratio-static("nettle-aes").

Static code size is the side of the ledger that this work has twice moved deliberately, and the aggregate hides both moves. Before shifts by a compile-time-known amount were unrolled, the same suite measured a geomean of #sym.times 4.94: a constant shift then cost one short loop regardless of the amount, whereas it costs one instruction per bit position once unrolled (@sc1-srl), and the geomean rose to #sym.times 7.13. The present edition brings it back to #sym.times #geo-static() --- but that near-cancellation is a coincidence of averaging two opposite effects, not a sign that nothing changed.

Both effects are confined to exactly the benchmarks whose code they concern, which is what makes the attribution safe. Making variable-count left shifts straight-line (@sc1-sll) converts trip count into code size, so the shift-heavy workloads grew again: `nettle-aes` by #sym.times 1.7 relative to its own earlier ratio, `edn` by #sym.times 1.5, `tarfind` and `depthconv` by #sym.times 1.4, `qrduino` by #sym.times 1.3. Resolving a sub-word access whose lane is known at compile time (@sc1-subword-const) moves the other way and moves further: a byte load that cost an `and`/shift/extract chain becomes a handful of instructions, so the byte- and halfword-heavy workloads collapsed --- `statemate` to #sym.times 0.18 of its earlier ratio, `sglib-combined` to #sym.times 0.45, `nsichneu` to #sym.times 0.74. Two benchmarks that do neither, `crc32` and `matmult-int`, are unchanged to the digit. The dynamic counts of @sec-embench-perf show the same two changes with the signs no longer opposed.
...
#figure(
  embench-perf-table,
  caption: [Embench-IoT dynamic retired-instruction counts on Spike and rvsc1 run-time overhead, #perf-stamp. Geomean is over all nineteen benchmarks.],
) <tbl-embench-perf>
...
The `rvsc2` column is also a check on the harness rather than only a baseline: `rvsc2` and `gcc17` are the same compiler differing in target configuration, so their counts should coincide, and over the suite they do to a geometric mean of 1.002. The one benchmark that visibly deviates, `statemate` at 1.03, does so for a reason outside the compiler --- the `rvsc*` triples take newlib's generic C string routines where the stock target takes its hand-written assembly ones (@sc1-linked-isa), and `statemate` moves enough bytes for the difference to show. The static comparison is unaffected, and stays exactly 1.000, because it measures the benchmark's own objects and no library code at all.

This aggregate should not be compared directly with the #sym.times 13.7 published before the constant-count unroll, because neither the compiler nor the set of benchmarks being averaged is the same. Several effects are superimposed. The unroll lowered the dynamic cost of compile-time-constant shifts, which is what allowed several of the most expensive benchmarks to finish inside the Spike budget at all; admitting those expensive workloads to the average pushes it up, independently of any benchmark becoming slower. The register fix removed work that had been pure waste: in that round `aha-mont64` fell from 413 815 964 retired instructions to 197 361 701 and `depthconv` from 1 212 764 378 to 593 772 817, each roughly halving, because a corrupted `t0` had been sending them down far longer paths while still, by luck, producing the right answer. The aliasing fix moved the figure the other way, since a synthesized sub-word access could no longer reuse a word its neighbour had already loaded: `statemate` nearly doubled, from #sym.times 59 to #sym.times 107. And this edition moved it back down, from #sym.times 22.2 to #sym.times #geo-dynamic(), by removing the run-time lane computation wherever the compiler can see the lane (@sc1-subword-const) and by making variable-count left shifts straight-line (@sc1-sll). Every benchmark improved or held: the largest single change is `statemate`, from #sym.times 107.2 to #sym.times #ratio-dynamic("statemate"), which is the aliasing cost being paid back --- what the fix forbade was reusing a loaded word, and a lane resolved at compile time barely loads one. The stable comparison is the per-benchmark ratio in the table, not the aggregate across editions of it.
