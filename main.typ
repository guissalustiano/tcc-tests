...
executados em cinco níveis de otimização: todas as 8 142 combinações que este alvo _freestanding_
...
compiled and executed at five optimization levels: all 8 142 combinations this freestanding target can
...
Every combination that runs, passes. "Skipped" denotes programs that fail to compile or link, or that are excluded up front as known-unsupported. Fifty-five sources are excluded at every optimization level, in the eight categories of @tbl-torture-skips. None of them is related to sc1's restricted instruction set: they are runtime-library gaps, language features absent from 32-bit RISC-V in general, host-specific inline assembly, or compile-budget exhaustion.
...
#figure(
  table(
    ...
    [C99 math runtime], [1], [Tests a narrowing GCC declines without one; fails identically on stock `rv32i`],
    [*Total*], [*55*], [],
  ),
  caption: [Torture-suite sources excluded at every optimization level, by cause],
) <tbl-torture-skips>

The two runtime-library categories, which together account for 42 of the 55, are both build configuration gaps rather than compiler or ISA issues. The linked newlib `libc.a`'s stdio wrappers (`printf`, `sprintf`, `fprintf`, and the `_chk`/`v`-prefixed variants) call internal reentrant symbols (`_vfprintf_r`, `_svfprintf_r`, `_vfiprintf_r`) that this newlib build never defines, so any test calling a `printf`-family function fails to link; likewise, no libm is built for this target, so the two tests calling `pow` and `floor` fail on the same kind of undefined reference. Three further sources --- `20030626-1.c`, `20030626-2.c` and `960327-1.c` --- hit the `printf` gap only at `-O0`, which is what raises that row of @tbl-torture-results to 58. Each calls `sprintf` with a literal format string and no varying conversion, so from `-O1` onwards GCC folds the call into the stores it is equivalent to and no reference to `sprintf` survives to be linked; at `-O0` the call remains and fails on the same undefined `_svfprintf_r`. The transformation is constant folding rather than dead-code elimination, as `20030626-1.c` tests the length `sprintf` returns and so cannot simply have the call discarded.
...
Correctness was established for all synthesis cases exercised by the test suites in rvsc0 and rvsc1. All ISA compliance tests pass; the behavioral tests confirm semantic equivalence on every program executed, including all 8 142 torture-suite combinations that this freestanding target can compile and link (the 278 it cannot are excluded for runtime-library and language-feature reasons unrelated to the instruction set). Reaching that point required three defects to be fixed --- an unreserved scratch register, an aliasing gap in the sub-word store synthesis, and hand-written `libgcc` assembly that bypassed the instruction restrictions entirely. The first accounted for all but two of the torture suite's 36 execution failures; none of the three was identified by the suites that were failing on them, and the last two were found only by the Embench measurement described above. With all three fixed, all nineteen benchmarks execute correctly. The rvsc2 target compiles the full Embench-IoT suite with code identical to the upstream GCC 17 baseline (geomean ratio = 1.000), confirming that the custom target configuration introduces no overhead relative to a stock build.
