...
#figure(
  table(
    ...
    [`edn`],            [68 624 356], [366 576 521],      [5.3#sym.times],
    ...
    [`nettle-aes`],     [4 674 247],  [81 995 045],      [17.5#sym.times],
    [`nettle-sha256`],  [4 917 617],  [115 138 783],     [23.4#sym.times],
    [`nsichneu`],       [2 514 373],  [14 682 354],       [5.8#sym.times],
    [`picojpeg`],       [3 602 642],  [178 740 510],     [49.6#sym.times],
    [`qrduino`],        [5 139 825],  [192 601 112],     [37.5#sym.times],
    [`sglib-combined`], [3 005 598],  [44 946 044],      [15.0#sym.times],
    [`slre`],           [2 983 191],  [55 833 329],      [18.7#sym.times],
    [`statemate`],      [2 065 673],  [221 508 876],    [107.2#sym.times],
    [`tarfind`],        [5 287 682],  [27 676 625],       [5.2#sym.times],
    [`ud`],             [6 622 451],  [8 379 607],        [1.3#sym.times],
    [`wikisort`],       [_linkfail_], [8 689 947],       [---],
    [`xgboost`],        [3 804 380],  [304 988 121],     [80.2#sym.times],
  ),
  caption: [Embench-IoT dynamic retired-instruction counts on Spike and rvsc1 run-time overhead, measured with the backend at revision `8a0646efee4`. Geomean is over the eighteen benchmarks that completed on both toolchains.],
) <tbl-embench-perf>

Across the eighteen benchmarks that completed on both toolchains, synthesis inflates the dynamic instruction count by a mean of #sym.times 13.9, close to double the #sym.times 7.13 static-size penalty, because the most expensive syntheses sit inside the hottest loops. The spread is wide and, as with code size, tracks each workload's reliance on synthesized instructions: `matmult-int` and `ud`, dominated by native multiply--add work, run at #sym.times 1.0 and #sym.times 1.3, whereas the byte-access-heavy `statemate` and the shift- and rotate-heavy `xgboost` reach #sym.times 107.2 and #sym.times 80.2. This confirms the pedagogical point quantitatively: the run-time cost of an absent instruction is not a fixed tax but is paid in proportion to how often the program actually needs it.
...
Correctness was established for all synthesis cases exercised by the test suites in rvsc0 and rvsc1. All ISA compliance tests pass, and the behavioral tests confirm semantic equivalence across all tested programs; the Embench measurement described above additionally found five programs the suites do not reach, traced to two backend defects --- an unreserved scratch register and an aliasing gap in the sub-word store synthesis --- both since fixed, after which all nineteen benchmarks execute correctly. The rvsc2 target compiles the full Embench-IoT suite with code identical to the upstream GCC 17 baseline (geomean ratio = 1.000), confirming that the custom target configuration introduces no overhead relative to a stock build.

The cost of synthesis was quantified on two axes. Statically, the synthesized rvsc1 target inflates code size by a mean of #sym.times 7.13 over the nineteen Embench benchmarks, with a range from #sym.times 1.42 (`ud`, few shifts) to #sym.times 22.16 (`statemate`, comparison-heavy control flow). Dynamically, it inflates retired instruction counts by a mean of #sym.times 13.9 over the eighteen benchmarks that completed on both toolchains; the nineteenth, `wikisort`, runs correctly under rvsc1 but has no rvsc2 baseline to compare against, since that configuration does not link.
