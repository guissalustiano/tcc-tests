...
#figure(
  table(
    ...
    [`edn`],            [3 244],  [40 332],  [12.43],
    [`huffbench`],      [2 140],  [12 372],  [5.78],
    [`matmult-int`],    [936],    [1 500],   [1.60],
    [`md5sum`],         [1 016],  [2 432],   [2.39],
    [`nettle-aes`],     [4 444],  [69 632],  [15.67],
    [`nettle-sha256`],  [6 976],  [135 040], [19.36],
    [`nsichneu`],       [19 668], [65 428],  [3.33],
    [`picojpeg`],       [15 360], [240 788], [15.68],
    [`qrduino`],        [12 852], [216 620], [16.85],
    [`sglib-combined`], [10 824], [76 856],  [7.10],
    [`slre`],           [4 256],  [38 060],  [8.94],
    [`statemate`],      [6 484],  [144 268], [22.25],
    [`tarfind`],        [528],    [3 320],   [6.29],
    [`ud`],             [1 436],  [2 032],   [1.42],
    [`wikisort`],       [7 744],  [41 488],  [5.36],
    [`xgboost`],        [624],    [7 912],   [12.68],
  ),
  caption: [Embench-IoT static code size (`.text` bytes, `-O2`) and rvsc1 expansion ratios. Measured with the backend at revision `0b3abf309dd`.],
) <tbl-embench-size>
...
One benchmark yields no measurement under rvsc1. `qrduino` runs to completion but fails its own output check at `-O2` and `-O3`, passing at `-O0`, `-O1` and `-Os`, and is reported as _fail_; it is excluded from the geomean. It compiles and verifies correctly under rvsc2 and under the unrestricted `gcc17` build from identical sources, so the defect is in the synthesis path and remains open. `wikisort` reports an rvsc1 count but no ratio, because the rvsc2 baseline for it does not link.

Four further benchmarks --- `edn`, `sglib-combined`, `wikisort` and `nsichneu` --- reached this table only after a backend fix that this measurement provoked. The first three trapped on Spike with a load from an address outside any allocated section, and the fourth exhausted the 300-second budget. All four shared one cause: the absolute-address sequences that stand in for `auipc`-relative jumps and calls hardcode `t0` inside raw assembly text, which the register allocator never sees, and `t0` had not been reserved against allocation the way its counterpart `t1` already was. A value living in `t0` across an ordinary loop back edge was therefore overwritten by the jump's own target address. Reserving the register restored all four; `nsichneu` in particular was never genuinely slow, and now retires 14 682 354 instructions, within 0.05% of what it retired before the defect was introduced.

Finding them at all was a consequence of the harness correction rather than of the benchmarks themselves, and it repeats the lesson of @sc1-torture-tests exactly. The earlier harness recorded a retired-instruction count even when the run exited nonzero, so an aborted benchmark entered the table as an ordinary number: `edn` was published at 512 612 605 retired instructions, an unremarkable-looking #sym.times 7.5 overhead. Once failed exits were recorded as failures, the same benchmark reported the 4 899 454 instructions it managed before trapping --- a nominal #sym.times 0.1, synthesis apparently outrunning the native baseline by a factor of fourteen. That impossibility is what exposed the crash, and through it a defect that had been corrupting results in every benchmark that used a jump.
...
#figure(
  table(
    ...
    [`aha-mont64`],     [12 997 204], [197 361 701],     [15.2#sym.times],
    [`crc32`],          [5 990 964],  [73 261 616],      [12.2#sym.times],
    [`depthconv`],      [54 552 560], [593 772 817],     [10.9#sym.times],
    [`edn`],            [68 624 356], [366 564 355],      [5.3#sym.times],
    [`huffbench`],      [2 398 918],  [80 332 721],      [33.5#sym.times],
    [`matmult-int`],    [24 445 649], [25 544 502],       [1.0#sym.times],
    [`md5sum`],         [2 917 201],  [38 739 114],      [13.3#sym.times],
    [`nettle-aes`],     [4 674 247],  [82 001 307],      [17.5#sym.times],
    [`nettle-sha256`],  [4 917 617],  [115 120 799],     [23.4#sym.times],
    [`nsichneu`],       [2 514 373],  [14 682 354],       [5.8#sym.times],
    [`picojpeg`],       [3 602 642],  [178 396 127],     [49.5#sym.times],
    [`qrduino`],        [5 139 825],  [_fail_],          [---],
    [`sglib-combined`], [3 005 598],  [44 936 682],      [15.0#sym.times],
    [`slre`],           [2 983 191],  [55 833 329],      [18.7#sym.times],
    [`statemate`],      [2 065 673],  [121 984 718],     [59.1#sym.times],
    [`tarfind`],        [5 287 682],  [27 676 625],       [5.2#sym.times],
    [`ud`],             [6 622 451],  [8 379 607],        [1.3#sym.times],
    [`wikisort`],       [_linkfail_], [8 689 947],       [---],
    [`xgboost`],        [3 804 380],  [304 936 921],     [80.2#sym.times],
  ),
  caption: [Embench-IoT dynamic retired-instruction counts on Spike and rvsc1 run-time overhead, measured with the backend at revision `0b3abf309dd`. Geomean is over the seventeen benchmarks that completed on both toolchains.],
) <tbl-embench-perf>

Across the seventeen benchmarks that completed on both toolchains, synthesis inflates the dynamic instruction count by a mean of #sym.times 12.6, close to double the #sym.times 7.13 static-size penalty, because the most expensive syntheses sit inside the hottest loops. The spread is wide and, as with code size, tracks each workload's reliance on synthesized instructions: `matmult-int` and `ud`, dominated by native multiply--add work, run at #sym.times 1.0 and #sym.times 1.3, whereas the shift- and rotate-heavy `xgboost` and the comparison-heavy `statemate` reach #sym.times 80.2 and #sym.times 59.1. This confirms the pedagogical point quantitatively: the run-time cost of an absent instruction is not a fixed tax but is paid in proportion to how often the program actually needs it.

This mean should not be compared directly with the #sym.times 13.7 published before the constant-count unroll, because neither the compiler nor the set of benchmarks being averaged is the same. Three effects are superimposed. The unroll lowered the dynamic cost of compile-time-constant shifts, which is what allowed several of the most expensive benchmarks to finish inside the Spike budget at all. Admitting those expensive workloads to the average pushes it up, independently of any benchmark becoming slower. And the register fix described above removed work that had been pure waste: `aha-mont64` fell from 413 815 964 retired instructions to 197 361 701 and `depthconv` from 1 212 764 378 to 593 772 817, each roughly halving, because a corrupted `t0` had been sending them down far longer paths while still, by luck, producing the right answer. The stable comparison is the per-benchmark ratio in the table, not the aggregate across editions of it.

== Discussion

The results establish correctness first and cost second. On the correctness axis, the ISA compliance tests confirm that the compiler never emits a forbidden mnemonic. Behavioral equivalence is established independently by execution on Spike: every rvsc1 program verifies its own results against independently computed values and exits 0, and every rvsc0 single-function program writes the corresponding success token to `tohost`. Synthesis therefore changes how a computation is expressed, not what it computes. That holds across every program in the three test suites, but not yet across every program: one of the nineteen Embench benchmarks, `qrduino`, still fails its own output check under rvsc1 at two optimization levels while verifying correctly under rvsc2 (@tbl-embench-perf), so the synthesis path retains at least one defect that neither the hand-written suites nor the torture corpus reaches. The suites bound where correctness has been demonstrated; they do not establish it everywhere.

The cost of that re-expression is quantified along two dimensions. Statically, synthesis inflates code size by a mean of #sym.times 7.13 over the Embench suite (@tbl-embench-size), dynamically, it inflates the retired-instruction count by a mean of #sym.times 12.6 over the benchmarks that complete (@tbl-embench-perf). The dynamic penalty is the larger of the two because the costliest syntheses of the variable-count shift loops, each of which re-materializes its own back-edge every iteration (@sc1-sll), tend to sit inside the hottest loops, so their cost is multiplied by trip count rather than merely by static occurrence. Both penalties vary by more than an order of magnitude across workloads, from near-parity for multiply--add-dominated code (`matmult-int`, `ud`) to two orders of magnitude for shift- and comparison-heavy code (`xgboost`, `statemate`).
...
Correctness was established for all synthesis cases exercised by the test suites in rvsc0 and rvsc1. All ISA compliance tests pass, and the behavioral tests confirm semantic equivalence across all tested programs; the Embench measurement described above additionally found five programs the suites do not reach, four of which were traced to a single unreserved scratch register and fixed, with the fifth still open. The rvsc2 target compiles the full Embench-IoT suite with code identical to the upstream GCC 17 baseline (geomean ratio = 1.000), confirming that the custom target configuration introduces no overhead relative to a stock build.

The cost of synthesis was quantified on two axes. Statically, the synthesized rvsc1 target inflates code size by a mean of #sym.times 7.13 over the nineteen Embench benchmarks, with a range from #sym.times 1.42 (`ud`, few shifts) to #sym.times 22.25 (`statemate`, comparison-heavy control flow). Dynamically, it inflates retired instruction counts by a mean of #sym.times 12.6 over the seventeen benchmarks that completed on both toolchains. One further benchmark, `qrduino`, fails its own output check under rvsc1 at `-O2` and `-O3` while verifying under rvsc2 --- an unresolved miscompilation in the synthesis path, reported in @tbl-embench-perf and excluded from the mean.
