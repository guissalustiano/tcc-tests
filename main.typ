...
// The Embench-IoT tables, chart and aggregates are built from the measurement
// CSVs rather than transcribed, so re-running the harness updates the numbers,
// the geometric means and the revision stamps in the captions together.
#import "figures/embench.typ": (
  embench, embench-overhead-figure, embench-perf-table, embench-size-table,
  geo-dynamic, geo-sc2-static, geo-static, perf-stamp, ratio-dynamic,
  ratio-static, retired, size-stamp,
)
...
// The book's own Figure 4.21, reproduced as an image rather than redrawn, so
// that a reader holding the book sees exactly the diagram the course works
// from. The credit line is mandatory: the illustration is the publisher's.
#diagram(
  image("figures/hannersy-patterson-implementation-fig4-21.png", width: 100%),
  caption: [The simple single-cycle datapath with the control unit, for the eight-instruction subset (`lw`, `sw`, `beq`, `add`, `addi`, `sub`, `and`, `or`)],
  legend: [
    Source: Figure 4.21 of @patterson2020 --- David A. Patterson and John L.
    Hennessy, _Computer Organization and Design: RISC-V Edition_, 2nd ed.,
    Morgan Kaufmann (an imprint of Elsevier), 2020. Reproduced here for
    academic purposes; copyright in the original illustration remains with the
    authors and the publisher.

    Note: black lines carry values and blue lines carry control signals. The
    subset has no upper-immediate path into the register file (`lui`), no
    PC~+~4 write-back into the register file (`jal`, `jalr`), no
    register#sym.arrow.r#h(0pt)PC path (`jalr`) and no path from the
    PC~+~immediate adder into the register file, which only steers the PC
    multiplexer (`auipc`); each of those absences is the hardware reason for a
    synthesis derived in @sc1-synthesis.
  ],
) <fig-datapath>

The two absences that matter most for a compiler are visible in the figure as
multiplexer inputs that are simply not there. Without an upper-immediate input to
...
Spike is the official RISC-V ISA reference simulator @spike. It implements the full RV64IMAFD instruction set, making it suitable as the behavioral oracle: a binary compiled by `rvsc1-unknown-elf-gcc` contains only valid RV32I instructions (the synthesis sequences are themselves valid RV32I), so Spike can execute it and report the correct result. The simulator is given a different `--isa` string depending on how the binary is run. rvsc0 binaries are bare-metal and execute under `--isa=rv32i`, exactly the ISA the target models. rvsc1 binaries run on top of the proxy kernel, and the proxy kernel itself uses CSR, atomic and fence instructions, so Spike must be widened to `--isa=rv32imac_zicsr_zifencei` for those runs. The user program still contains only sc1 instructions, but that is a property the simulator no longer checks, which is why it is established statically instead (@strategy-isa). `--log-commits` counts retired instructions for the performance measurements.
...
Composing a branch out of `[slt]` this way costs less than `[slt]` alone: @tab-synthesis-cost lists `blt`/`bltu` at 16 instructions against 49 for `slt`. The apparent arithmetic error is real behavior, and its cause is that a branch never needs the comparison as a *value*. The last step of the `[slt]` expansion is a logical right shift by 31, isolating the sign bit into bit 0 so that the result is exactly 0 or 1. With no native `srl` that shift is itself synthesized (@sc1-srl), and its dominant cost is constructing the mask $2^31$ by a chain of 31 `add rd, rd, rd` doublings. When the only consumer of the result is a test against zero, GCC's combine pass rewrites `(x >> 31) != 0` as `(x & 0x80000000) != 0`; the mask is then an ordinary constant that sc1 materializes with a single `lui`, and the doubling chain disappears. Compiled at `-O2` for rvsc1, `return a < b;` emits 49 instructions, 32 of which are the `li` and the 31 doublings that build the shift mask; `if (a < b) g = 1;` over the same operands emits 18, contains no doubling chain, and tests the sign bit with `li a7, -2147483648`; `and`; `beq`. The comparison rows of the cost table are thus worst cases for the value-producing form only, and no ordered branch pays them.
...
This algorithm makes no assumption about the address: the offset is recomputed at run time from the pointer itself, so it is correct for a pointer whose value the compiler never sees.
...
Since sc1 shifts are themselves synthesized via `add`/`beq` loops, and the extraction above uses four of them, a byte load whose lane is unknown retires up to 460 instructions and a halfword load up to 532 (@tab-synthesis-cost). The worst case is an *aligned* address rather than a misaligned one: the smaller the byte offset, the smaller the extraction shift amount, and the more bit positions the synthesized `srl` has left to walk.

===== Lanes known at compile time <sc1-subword-const>

Most sub-word accesses in ordinary code are not through an opaque pointer. A global `char`, a `short` field of a struct, a byte of a local array at a constant index, in each of these the object's position within its containing word is fixed when the program is compiled, and paying 460 instructions to rediscover it at run time is pure waste. Before emitting the sequence above, the expansion therefore asks whether the lane is a compile-time constant, and if it is, emits the word access and a fixed extraction instead: the whole `addr & 3`, multiply-by-eight and variable shift disappear, and with them the synthesized shift that dominated the cost.

What makes the lane knowable is alignment, and the two facts available about it are not symmetric. `MEM_ALIGN` is a lower bound, so it settles the question in one direction only: at least word alignment means lane 0, but an alignment of 16 bits means lane 0 _or_ lane 2 and settles nothing. For a non-zero lane the residue comes instead from the front end's own record of the address --- a modulus $M$ and remainder $N$ with the address congruent to $N$ modulo $M$, which pins the lane exactly when $M$ is at least a word. Any offset the compiler cannot resolve this way falls back to the run-time path rather than to a guess, since the failure mode of guessing is an unaligned `lw` or the wrong word entirely.

Two consequences are worth stating because they shaped the result. First, the extraction is chosen per lane rather than written once: a right shift costs roughly in proportion to the number of bits that survive it, while a left shift costs one `add` per position, so lane 0 masks the byte off directly while lanes 1 to 3 shift the byte up to the top of the word and take a single logical `>>24`, the cheapest right shift available, since only eight bits survive it. Second, the sequence has to be built by emitting the two shifts directly rather than by writing the equivalent C-level expression: `(w << 16) >> 24` is canonicalized straight back into `(w >> 8) & 0xFF` by the middle end, undoing the choice, whereas an expansion that emits each shift itself is past the point where anything can rewrite it.

For this to reach plain scalars, the targets that need it also raise the alignment of every static and stack object to a word. Upstream's rule widens only aggregates, which left a bare `char` global byte-aligned and therefore permanently on the slow path; the cost of widening is at most three bytes of padding per object. It is applied only where the definition is visible, so an `extern char` defined by a library built without the rule still reads as byte-aligned and takes the run-time path, which is the conservative direction.

=== Stores <sc1-stores>

==== SB and SH <sc1-sb>

A byte store (`sb`) or halfword store (`sh`) is implemented as a read-modify-write: load the word containing the target location, clear the target bits, insert the new value, and write back. The sequence below computes the bit offset at run time, for a base address the compiler cannot resolve; when it can, the same compile-time fold described in @sc1-subword-const applies, and the offset becomes a constant.
...
#figure(
  table(
    ...
    [`lb`/`lbu`],    [rvsc0, rvsc1], [77 known lane, 460 unknown],  [2],
    [`lh`/`lhu`],    [rvsc0, rvsc1], [85 known lane, 532 unknown],  [2],
    [`sb`],          [rvsc0, rvsc1], [32 known lane, 95 unknown],   [3],
    [`sh`],          [rvsc0, rvsc1], [23 known lane, 82 unknown],   [3],
    [`jump`],        [rvsc0],        [1],    [0],
    [`jump`],        [rvsc1],        [3],    [1],
    [`jal`],         [rvsc1],        [5],    [1],
    table.hline(),
...
Counts in @tab-synthesis-cost are dynamic: instructions retired for the worst-case operand, which is the quantity @sec-embench-perf draws on. They are measured rather than derived, by calling a one-operation function in a loop and differencing the retired-instruction count between two trip counts, so that the loop and call overhead cancels exactly. Two things about the worst case are easy to get wrong and were determined by scanning rather than assumed. The cost of a shift depends on the data as well as the amount, because the extraction step for each bit position is a conditional merge: `srl` by 1 costs 158 instructions on an all-ones operand but 127 on zero. And sub-word loads are most expensive at an *aligned* address, not a misaligned one, because the extraction shift gets cheaper as the offset grows, an unknown-lane `lb` costs 460 instructions at offset 0 and 244 at offset 3, the opposite of what "worst-case misaligned access" suggests.

The four sub-word rows carry two figures each because they are two different sequences, not one sequence with a spread of operands (@sc1-subword-const). The *unknown* column is an access through a pointer the compiler cannot resolve, and is what the harness measures by passing the address in as a parameter. The *known lane* column is the same source construct applied to a global, where the position within the word folds at compile time; it is itself lane-dependent, and the figure quoted is the worst lane. The best lane is in a different league again: a word-aligned `lbu` costs 2 instructions and an `lhu` 3, against 460 and 532 unknown. Which of the two a program pays is not a property of the operation but of what the compiler can see about the address, which is why the Embench measurements move so unevenly between benchmarks (@sec-embench-size).
...
#figure(
  table(
    ...
    [Sub-word aliasing], [Adjacent `unsigned char` and `unsigned short` objects sharing one word, stored through unbarriered `noinline` functions: neither read-modify-write may resurrect its neighbour's stale value --- the regression test for the `qrduino` miscompilation (@sc1-corner-cases)],
    [Values live across jumps], [Twenty-four accumulators carried through a branching loop, enough live values to fill the register file so that several sit in scratch registers across a synthesized jump --- the regression test for the unreserved `t0` (@sc1-corner-cases)],
  ),
  caption: [Behavioral test programs for rvsc1, by operation category],
) <tbl-sc1-behav-files>

All 8 behavioral tests pass at every optimization level (`-O0` through `-Os`), for 8 × 5 = 40 cases. The last two are regression tests rather than coverage of an operation: each was written after a defect that the rest of this suite had run past.
...
The `c99_runtime` group is the one place where reading the directives costs coverage rather than adding it. Two of its three sources, `pr78622.c` and `pr79327.c`, pass. They are skipped anyway, because the requirement they declare genuinely is not met: GCC treats this target as lacking a C99 runtime, which is what `20030125-1.c` fails on. That source asserts that GCC narrows `floor` on a `float` argument to `floorf`, and defines a weak `floor` that aborts if the narrowing did not happen; without a C99 math runtime GCC correctly declines to narrow, so the weak function runs and aborts. That this is unrelated to the restricted instruction set is settled by comparison rather than by argument: a stock `rv32i` toolchain fails the same two optimization levels and passes the same three.

The static mnemonic check runs over the same suite on a scope of its own, because it never executes the program: a source that needs a filesystem or a C99 math runtime still compiles, and disassembling its object says exactly as much about the compiler as any other, so the compile budget is the only reason to leave a source out. On that scope, 8 395 combinations are attempted and #emph[8 360 produce an object file containing only the ten instructions sc1 implements]. The 35 that produce no object are front-end rejections spanning four distinct causes: `__int128` (15), `sys/mman.h` (10), x87-specific inline asm (5), and `_Decimal` floating point (5), all of which @tbl-torture-skips attributes to the freestanding runtime or to 32-bit RISC-V in general rather than to sc1. Extending the check to the whole 8 420 with the `--include-unsupported` form, worth doing because a program that cannot link can still crash the compiler, adds the five upstream-flagged "expensive" sources and yields 8 368 objects, every one ISA-clean, still with no ICE, plus 17 exhausted compile budgets confined to those five sources. That last count is sensitive to machine load, as the affected compilations sit near the 120-second budget, the ISA-compliance and ICE counts are not, and are the meaningful results.

The failure list of the behavioral sweep is committed alongside the harness, so that the next sweep diffs against it rather than scrolling past. Every one of the 8 345 combinations that compiles, links and runs terminates with the exit status its assertions require.
...
- *rvsc2*: the native RV32I target of this work, in which nothing is synthesized.
- *rvsc1*: the synthesized target.
...
Across the suite, synthesis inflates code size by a mean of #sym.times #geo-static() relative to the `gcc17` baseline. The custom `rvsc2` target produces code identical to `gcc17` for all 19 benchmarks. The per-benchmark ratio tracks how much of each workload's own code the target cannot express directly: `ud`, which performs most of its arithmetic inside library routines rather than in inline shifts, expands only #sym.times #ratio-static("ud"), whereas `nettle-aes`, whose round function is a chain of constant-amount rotations and byte extractions that each unroll into straight-line code, expands #sym.times #ratio-static("nettle-aes").
...
Across all nineteen benchmarks, synthesis inflates the dynamic instruction count by a geometric mean of #sym.times #geo-dynamic(), more than twice the #sym.times #geo-static() static-size penalty, because the most expensive syntheses sit inside the hottest loops. The spread is wide and, as with code size, tracks each workload's reliance on synthesized instructions: the cheapest cases are `matmult-int` at #sym.times #ratio-dynamic("matmult-int"), `nsichneu` at #sym.times #ratio-dynamic("nsichneu") and `statemate` at #sym.times #ratio-dynamic("statemate"), whereas the shift- and rotate-heavy `xgboost` and the byte-scanning `tarfind` reach #sym.times #ratio-dynamic("xgboost") and #sym.times #ratio-dynamic("tarfind"). `matmult-int` and `ud` are no longer the near-parity cases they were in earlier editions of this measurement: at #sym.times #ratio-dynamic("matmult-int") and #sym.times #ratio-dynamic("ud") they now carry the cost of synthesized multiply and divide in `libgcc`, which earlier measurements had avoided by linking assembly the target cannot execute. This confirms the pedagogical point quantitatively: the run-time cost of an absent instruction is not a fixed tax but is paid in proportion to how often the program actually needs it.

== Discussion

The results establish correctness first and cost second. On the correctness axis, the ISA compliance tests confirm that the compiler never emits a forbidden mnemonic. Behavioral equivalence is established independently by execution on Spike: every rvsc1 program verifies its own results against independently computed values and exits 0, and every rvsc0 single-function program writes the corresponding success token to `tohost`. Synthesis therefore changes how a computation is expressed, not what it computes. That now holds across every program the three test suites actually execute: 95 ISA-compliance cases, 40 behavioral cases, and 8 345 of the 8 420 torture combinations, the remaining 75 being programs that declare a prerequisite this freestanding target does not provide, or that do not compile, for reasons unrelated to the instruction set (@tbl-torture-skips) and across all nineteen Embench benchmarks. It did not hold before this measurement. The torture suite was failing 36 combinations, and two further defects lay outside what any suite reached at all. The 36 were attributed only afterwards, by rebuilding the compiler with each candidate fix reverted in turn until the failure set reappeared; the other two were found by running a fourth, independent corpus and by refusing to accept a failed run as a number. The suites bound where correctness has been demonstrated, and widening the corpus widened the bound.

The cost of that re-expression is quantified along two dimensions. Statically, synthesis inflates code size by a geometric mean of #sym.times #geo-static() over the Embench suite (@tbl-embench-size); dynamically, it inflates the retired-instruction count by a geometric mean of #sym.times #geo-dynamic() (@tbl-embench-perf). The dynamic penalty is the larger of the two because the costliest syntheses, the variable-count shifts and the unknown-lane sub-word accesses built on them (@sc1-srl), tend to sit inside the hottest loops, so their cost is multiplied by trip count rather than merely by static occurrence. Both penalties vary by more than an order of magnitude across workloads: statically from #sym.times #ratio-static("ud") (`ud`) to #sym.times #ratio-static("nettle-aes") (`nettle-aes`), and dynamically from #sym.times #ratio-dynamic("matmult-int") (`matmult-int`) to #sym.times #ratio-dynamic("xgboost") (`xgboost`). @fig-embench-overhead plots the two side by side on a logarithmic axis, where that spread, and the fact that the two orderings differ, is the shape of the whole result.

#diagram(
  embench-overhead-figure,
  caption: [Static and dynamic cost of rvsc1 synthesis, per benchmark],
  legend: [Each benchmark contributes two ratios against an unsynthesized build of the same source: `.text` bytes (`rvsc1 / gcc17`, @tbl-embench-size) and retired instructions (`rvsc1 / rvsc2`, @tbl-embench-perf). The axis is logarithmic, so equal distances are equal factors and #sym.times 1 is parity; benchmarks are ordered by dynamic overhead. The two orderings are not the same one: `edn` is the only benchmark that costs more in size than in time, `nettle-aes` pays almost the same factor on both, and `ud` pays #sym.times #ratio-static("ud") in size against #sym.times #ratio-dynamic("ud") in time.],
) <fig-embench-overhead>
...
Finally, the target configuration and synthesis infrastructure developed here could be packaged as a course resource for PCS3225, including pre-built toolchain binaries, startup files, linker scripts, and a simple Makefile that lets students go from a C source file to a Spike execution with a single command.
