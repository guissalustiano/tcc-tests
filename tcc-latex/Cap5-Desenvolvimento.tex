...
\texttt{gcc/config/riscv/riscv.cc}  & Target hooks: constant pool handling for LUI synthesis \\
...
are impossible; LUI synthesis is primarily needed to materialize large constants or absolute memory
addresses.

\textbf{Derivation.} Since \texttt{addi} encodes only 12-bit signed immediates (range $-2048$ to
$2047$), a 20-bit immediate cannot be loaded in one instruction. The constant-pool approach stores
the full 32-bit value (\texttt{imm20 << 12}) in the \texttt{.rodata} section at link time and
replaces the entire sequence with a single load:

\begin{lstlisting}
lw    rd, pool_entry(x0)  # rd = *(pool_entry)
\end{lstlisting}

This requires that \texttt{pool\_entry} fits in a 12-bit signed offset from \texttt{x0} (address $<
2048$), which holds for the short programs typical of the educational processor. The constant-pool
approach was chosen over a multi-instruction \texttt{addi}+shift sequence because it produces a
single instruction at each use site and avoids shift instructions that rvsc0 does not support.
