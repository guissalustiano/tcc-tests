...
The programs are self-validating --- they call \texttt{exit(0)} on success and \texttt{abort()} on
failure --- so no reference compiler is needed. The harness compiles each program at all five
...
Across the suite, synthesis inflates code size by a geometric mean of $4.94\times$ relative to the
\texttt{gcc17} baseline. The custom \texttt{rvsc2} target produces code identical to \texttt{gcc17}
for all 19 benchmarks (geomean ratio = 1.000), confirming that the rvsc2 configuration adds no
overhead relative to the upstream GCC 17 build.
...
\subsection{Individual Operation Cost} \label{sec:op-cost}

The variable left shift is the clearest illustration of synthesis cost, since its length grows with
the operand. As derived in Section~\ref{sec:sc1-sll}, the synthesized loop costs $6b + 1$ retired
instructions for a masked shift amount $b \geq 1$ (and 3 when $b = 0$).
Table~\ref{tab:sll-perf} compares this model against the measured isolated instruction count; the two
agree exactly.
...
\begin{tabular}{rrrr}
\toprule
\textbf{Shift amount $b$} & \textbf{rvsc2 (native)} & \textbf{rvsc1 model ($6b+1$)} & \textbf{rvsc1 (measured)} \\
\midrule
 0 & 1 &   3 &   3 \\
 1 & 1 &   7 &   7 \\
 8 & 1 &  49 &  49 \\
...
Table~\ref{tab:op-perf} reports two representative fixed-length cases.
...
\begin{tabular}{lrrr}
\toprule
\textbf{Operation} & \textbf{rvsc2 (native)} & \textbf{rvsc1 (synth)} & \textbf{Overhead} \\
\midrule
\texttt{xor} (register) & 1 &   3 &   3$\times$ \\
\texttt{sb} (byte store) & 1 & 113 & 113$\times$ \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Whole-Program Dynamic Instruction Counts} \label{sec:embench-perf}

The isolated figures above bound the cost of one operation; the dynamic cost of a whole program
depends on how often each synthesized instruction executes at run time, weighted by loop trip counts
rather than by static frequency. Five benchmarks exceeded a 300-second Spike budget under rvsc1 and
are reported as TIMEOUT.
...
instruction count by a geometric mean of $13.7\times$, roughly triple the $4.94\times$ static-size
...
tests confirm that the compiler never emits a forbidden mnemonic: every generated object was
disassembled with \texttt{objdump -M no-aliases}, which expands pseudo-instructions to their
underlying encodings, so a restricted instruction cannot slip past the allowlist disguised as a
pseudo. Behavioral equivalence is established independently by differential execution on Spike: every
...
code size by a geometric mean of $4.94\times$ over the Embench suite
...
penalty is the larger of the two because the costliest syntheses --- the shift loops, each of which
...
code size by a geometric mean of $4.94\times$ over the nineteen Embench benchmarks, with a range
from $1.41\times$ (\texttt{ud}, few shifts) to $16.39\times$ (\texttt{statemate},
comparison-heavy control flow). Dynamically, it inflates retired instruction counts by a geometric
...
--- extend the hardware, recompile, measure the speedup --- is a natural next step.
