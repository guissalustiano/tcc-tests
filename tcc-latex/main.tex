...
\begin{dedicatoria}
   ...
   \centering
   \noindent
   \textit{Para que servem tantos códigos, se a vida não é programada\\
   e as melhores coisas não têm lógica.} \vspace*{\fill}
\end{dedicatoria}
% ---
...
completo rv64imafd --- e produz binários ELF \textit{bare-metal} executáveis no simulador de ISA Spike.
...
checagem de cada mnemônico contra uma lista de permissão por alvo; a equivalência comportamental é
verificada executando no Spike programas de teste autovalidantes, que conferem cada resultado contra
...
against a per-target allowlist; behavioral equivalence is verified by executing self-validating test
programs on Spike, each of which checks its computed results against values derived independently
